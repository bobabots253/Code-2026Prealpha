...
public class GyroIOPigeon2 implements GyroIO {
  ...
  public GyroIOPigeon2() {
    ...
    // Disables all status signals not explicitly given an update frequency (does this do anything
    // in current code if both status signals have an update freq?)
    pigeon.optimizeBusUtilization();
    // Creates a queue for timestamp data (that can be fed into the odometryYawTimestamps array
    // later) using the SparkOdemetryThread file
    yawTimestampQueue = SparkOdometryThread.getInstance().makeTimestampQueue();
    // Creates a queue for yaw position data (that can be fed into the odometryYawTimestamps array
    // later) using the SparkOdemetryThread file, which is returned as a double (what does '::'
    // mean?)
  ...
  public void updateInputs(GyroIOInputs inputs) {
    // returns 'connected' as true if yaw and yawvelocity StatusSignals are able to be refreshed by
    // the Gyro with OK signals
    ...
    // clears the queues of all data. This will keep code processing optmized since the queues no
    // longer need to store data that has been moved to arrays
...
// 3. Use defined Status Signals to collect data -> Store it in queues -> put it into arrays for
// more permanent storge, clear queues after
